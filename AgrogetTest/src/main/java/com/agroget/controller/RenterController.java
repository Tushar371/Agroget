...
public class RenterController {
	...
	public ModelAndView logout(HttpSession session,SessionStatus status) {
		...
		mv.setViewName("index");
		status.setComplete();
		System.out.println("object removed from session scope  and request scope");
		return mv;
	}
	...
	public ModelAndView deleteEquipments(@RequestParam int equipmentid,Model model)
	{
		ModelAndView mv = new ModelAndView();
		equipmentDao.deleteequip(equipmentid);
	 //   renterdao.deleteEquipment(equipmentid);
	//	model.addAttribute("currentrenter", "/ProfileMyEquipment");
